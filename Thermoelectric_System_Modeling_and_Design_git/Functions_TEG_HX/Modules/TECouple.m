% Function Ouputs Power and Heat Flow through Thermoelectric unicouple 
% Material Properties : Bi2Te3
% DBC : Version 2 [ Smaller Dimensions] 

% Last Updated = 06/20, Thomson Heat Added. Qhin Changed, Total Power from
% Legs changed

  function [Qhin ,P, LH, LC]  = TECouple(Th, Tc)
...
% RL = 0:0.001:0.2;
% hnv = (0.1*10^-3):(0.1*10^-3):(1*10^-3)
% Tcv = [22.74, 22.99, 23.29, 23.65, 24.12, 24.71,25.51, 26.62,28.313,31.159]
...
thpf = int(thopf); % Integral of temperature dependent Thomson Coefficient
 ...
 %%
syms kn kp sn sp pp pn thn thp x % k: Conductivity, s: Seebeck, p: resistivity, p: p-leg, n:n-leg
...
thpm =  (thp(Th) - thp(Tc))./(Th-Tc);
thpm = double(thpm);
thnm =  (thn(Th) - thn(Tc))./(Th-Tc);
thnm = double(thnm);
...
%      I2 = 4.7;
    %
    Qhp(j) = spm(j).*Tempr(j+C2)*I2 + cp(j).*(Tempr(j+C2)-Tempr(j+C2+1))-(0.5).*(I2.^2).*rp1(j) ;%- 0.5.*thpm(j)*I2;  % Heat input to hot side
    Qcp(j) = spm(j).*Tempr(j+1+C2).*I2 + cp(j).*(Tempr(j+C2)-Tempr(j+1+C2))+ 0.5.*(I2.^2).*rp1(j);% + 0.5.*thpm(j)*I2; % Heat rejection from cold side
    Pp(j) = (Qhp(j)- Qcp(j)) ;% Power output per leg
   

    Qhn(j) = abs(snm(j)).*Tempr(j+C2).*I2 + cn(j).*(Tempr(j+C2)-Tempr(j+1+C2)) - (0.5).*(I2.^2).*rn1(j);%- 0.5.*(thnm(j))*I2 ; % Heat input to hot side
    Qcn(j) = abs(snm(j)).*Tempr(j+1+C2).*I2 + cn(j).*(Tempr(j+C2)-Tempr(j+1+C2)) + 0.5.*(I2.^2).*rn1(j);% + 0.5.*((thnm(j)))*I2 ;% Heat rejection from cold side
...
TPp =sum(Pp);% + Jhc2/2;  %-(Jhc1./2)-Jhc2;  % Total Power - P-Leg: Reduce half the Joule Heat from Top Plate from Qh and add the Joule Heat from the bottom to Qc
TPn =sum(Pn);% + Jhc2/2; %-(Jhc1./2)-Jhc2;% Total Power - N-Leg
%%
...
V_oc = (S2*(LH-LC));
% R_in = linspace(0, 0.1, 500);
% P_in = V_oc^2.*R_in ./ (repmat(RL2,1,500)+R_in).^2;
% P_el = max(P_in);
% Power = mean ([P_el, P]);
% 
% Pv(a) = P;
