...
% Function calcultes the Power Output, Heat Transferred, Voltage and
% Current for a complete unicouple

 function [QhinT, PT, VoltageT, I2] = CompleteUnicouple(Th,Tc,Couples)


global hn hp tp tn lp ln
% Naming Convention
% property/leg/otherconsideration   no slashes

%  Th = [200 300 400 500 600 ];% [C] Hot side temperature
m =100; % m is the number of segments, Set m >1
% RL = 0:0.001:0.2;
...
% hn = (0.3*10^-3) ; % [m] n-leg height 
ihn = hn;
% ln = 1.301*10^-3; %  [m] n-leg length
% tn = 1.307*10^-3; %  [m] n-leg thickness
% hp = (0.3*10^-3); %  [m] p-leg height
ihp = hp;
% lp = 1.301*10^-3; % [m] p-leg length
% tp = 1.307*10^-3 ; % [m] p-leg thickness
...
syms x
pnf = int(-1.89504849E-17*x^4 - 5.64823793E-13*x^3 + 1.48855690E-10*x^2 + 3.25428763E-08*x + 9.14809738E-06); %int(3.79548552E-22*x^6 - 6.75630691E-19*x^5 + 4.62762403E-16*x^4 - 1.59187729E-13*x^3 + 2.48272530E-11*x^2 + 4.34512734E-09*x + 6.47172345E-06);
% Bi2Te3 = int(-1.89504849E-17*x^4 - 5.64823793E-13*x^3 + 1.48855690E-10*x^2 + 3.25428763E-08*x + 9.14809738E-06);
% Seebeck N
snf = int(3.28019971E-14*x^4 - 1.66155332E-11*x^3 + 4.62835101E-09*x^2 - 6.11524875E-07*x - 1.76396851E-04); %int(-3.07307824E-20*x^6 + 5.17273632E-17*x^5 - 3.14308248E-14*x^4 + 8.23370332E-12*x^3 - 6.95427569E-10*x^2 - 2.15931357E-07*x - 1.30728621E-04);
%Bi2Te3 =int(3.28019971E-14*x^4 - 1.66155332E-11*x^3 + 4.62835101E-09*x^2 - 6.11524875E-07*x - 1.76396851E-04);

%  Thermal Conductivity N
knf = int(-1.25842544E-10*x^4 + 8.58521996E-08*x^3 - 5.60764755E-06*x^2 - 8.78714664E-04*x + 1.28916143E+00); %int( -3.42296835E-16*x^6 + 5.14867564E-13*x^5 - 1.96473071E-10*x^4 - 4.33833033E-08*x^3 + 4.91278625E-05*x^2 - 1.38542448E-02*x + 5.56989213E+00);
%Bi2Te3 =int(-1.25842544E-10*x^4 + 8.58521996E-08*x^3 - 5.60764755E-06*x^2 - 8.78714664E-04*x + 1.28916143E+00);

% Resisitivity P

ppf = int( -1.28936729E-15*x^4 + 1.18635837E-14*x^3 + 1.38861195E-10*x^2 + 4.81304310E-08*x + 8.63224270E-06); % int(1.26762914E-22*x^6 - 2.35719632E-19*x^5 + 1.60899624E-16*x^4 - 5.53161599E-14*x^3 + 1.53924136E-11*x^2 + 4.75622122E-09*x + 1.87165436E-06);
%Bi2Te3 =int( -1.28936729E-15*x^4 + 1.18635837E-14*x^3 + 1.38861195E-10*x^2 + 4.81304310E-08*x + 8.63224270E-06);


% Seebeck P
 
 spf=  int( -2.16011384E-14*x^4 + 1.21036642E-11*x^3 - 4.24902245E-09*x^2 + 7.16773656E-07*x + 1.75862621E-04); %int(-1.36579995E-20*x^6 + 2.24957017E-17*x^5 - 1.38487487E-14*x^4 + 3.96125625E-12*x^3 - 5.38600018E-10*x^2 + 2.14193134E-07*x + 6.98593256E-05);
%Bi2Te3 =int( -2.16011384E-14*x^4 + 1.21036642E-11*x^3 - 4.24902245E-09*x^2 + 7.16773656E-07*x + 1.75862621E-04);

 % Thermal Conductivity P
 
 kpf = int( -1.92004259E-10*x^4 + 1.19050111E-07*x^3 - 1.45459874E-05*x^2 - 4.95909112E-04*x + 1.10059563E+00); %int( -8.25275365E-16*x^6 + 1.50160725E-12*x^5 - 1.02862881E-09*x^4 + 3.18113901E-07*x^3 - 3.14899647E-05*x^2 - 9.18620390E-03*x + 7.73547497E+00);
  %Bi2Te3 =int( -1.92004259E-10*x^4 + 1.19050111E-07*x^3 - 1.45459874E-05*x^2 - 4.95909112E-04*x + 1.10059563E+00);

 % Thomson N  - New code 
 
 dsdtnf = symfun(diff(5.48885661E-14*x^3 + 2.22674400E-10*x^2 - 2.66412731E-07*x - 1.33513735E-04,x),x); % Derivative of seebeck with respect to T
 thonf = symfun( (dsdtnf),x); % Temeperature dependent Thomson Coefficient
 thnf =int(thonf); % Integral of temperature dependent Thomson Coefficient

  % Thomson P  - New code 
dsdtpf = symfun(diff( 1.50803298E-13*x^3 - 3.37285775E-10*x^2 + 3.01002672E-07*x + 1.23997961E-04,x),x); % Derivative of seebeck with respect to T
thopf = symfun((dsdtpf),x);   % Temeperature dependent Thomson Coefficient
thpf = int(thopf); % Integral of temperature dependent Thomson Coefficient
 ...
 % New Code
 pcf = int(2.05151220E-22*x^4 + 9.09366950E-18*x^3 + 1.05886347E-15*x^2 + 6.74632702E-11*x + 1.54345901E-08); 
...
ResT3_Act = t3t/t3_A;
ResT3_Sim = t3t/b2_A;
t3tf = ResT3_Act/ResT3_Sim;

ResT2_Act = t2t/t2_A;
ResT2_Sim = t2t/b2_A;
t2tf = ResT2_Act/ResT2_Sim;

ResT1_Act = t1t/t1_A;
ResT1_Sim = t1t/b2_A;
t1tf = ResT1_Act/ResT1_Sim;

ResL_Act = hp/LA;
ResL_Sim = hp/b2_A;
lf = ResL_Act/ResL_Sim;

ResB1_Act = b1t/b1_A;
ResB1_Sim = b1t/b2_A;
b1tf = ResB1_Act/ResB1_Sim;

ResB2_Act = b2t/b2_A;
ResB2_Sim = b2t/b2_A;
b2tf = ResB2_Act/ResB2_Sim;

ResB3_Act = b3t/b3_A;
ResB3_Sim = b3t/b2_A;
b3tf = ResB3_Act/ResB3_Sim;
...
hn = lf*(1.69*10^-3) ; % [m] n-leg height 
hp = lf*(1.69*10^-3); %  [m] p-leg height
t3t = t3tf*0.2032*10^-3;
t2t = t2tf*0.635*10^-3;
t1t = t1tf*0.2032*10^-3;
b3t = b3tf*0.2032*10^-3;
b2t = b2tf*0.635*10^-3;
b1t = b1tf*0.2032*10^-3;
...
% New code
thn = symfun( thnf, x); % Temeperature dependent t for n-leg
thp = symfun( thpf, x); % Temeperature dependent t for n-leg
...
TotH = t3t + t2t + t1t + b3t +b2t + b1t +hp;


HR1 = round(t3t/TotH,2);
HR2 = round(t2t/TotH,2);
HR3 = round(t1t/TotH,2);
HR4 = round(hp/TotH,2);
HR5 = round(b1t/TotH,2);
HR6 = round(b2t/TotH,2);
HRT = HR1 + HR2 + HR3 + HR4 + HR5 + HR6;
HR7 = round( 1 - HRT,2);
...
while (m1 < 3)| (m2 < 3) | (m3 < 3) | (m4 < 3) | (m5 < 3) | (m6 < 3)| (m7 < 3)
    if (m1 < 3)
        m1 = m1+1;
        m4 = m4 -1;
    elseif (m2 < 3)
...
% kt = [ kt7 kt6 kt5 kt4 kt3 kt2 kt1]';
% Mid-point K's
...
shp = hp./(m4); % Adjusts the element size to obtain accuarate Joule heat
shn = hn./(m4); % Adjusts the element size to obtain accuarate Joule heat
...
thpm = (thp(Tempr(C2+1)) - thp(Tempr(C3)))./(Tempr(C2+1) -Tempr(C3));
thpm = double(thpm);
thnm =  (thn(Tempr(C2+1)) - thn(Tempr(C3)))./(Tempr(C2+1) -Tempr(C3));
thnm = double(thnm);

pcmt =  (pc(Tempr(C1+1)) - pc(Tempr(C2)))./(Tempr(C1+1) -Tempr(C2));
pcmt = double(pcmt);

pcmb =  (pc(Tempr(C3+1)) - pc(Tempr(C4)))./(Tempr(C3+1) -Tempr(C4));
pcmb = double(pcmb);

RBp = (pcmb.* b1w)./(b1l.*b1t);
RBn = (pcmb.* b1w)./(b1l.*b1t);
RC = (pcmt* t1w)/(t1l*t1t);
...
%     shp = hp/(Co);
%     shn = hn/(Co);
    thpm(j) =  (thp(Tempr(j+C2)) - thp(Tempr(j+1+C2)))./(Tempr(j+C2)-Tempr(j+1+C2)); % new code
    thpm(j) = double(thpm(j));
    thnm(j) =  (thn(Tempr(j+C2)) - thn(Tempr(j+1+C2)))./(Tempr(j+C2)-Tempr(j+1+C2)); % new code
...
%      I2 = 4.7;
    %
    Qhp(j) = spm(j).*Tempr(j+C2)*I2 + cp(j).*(Tempr(j+C2)-Tempr(j+C2+1))-(0.5).*(I2.^2).*rp1(j) - 0.5.*thpm(j)*I;  % Heat input to hot side
    Qcp(j) = spm(j).*Tempr(j+1+C2).*I2 + cp(j).*(Tempr(j+C2)-Tempr(j+1+C2))+ 0.5.*(I2.^2).*rp1(j) + 0.5.*thpm(j)*I; % Heat rejection from cold side
    Pp(j) = (Qhp(j)- Qcp(j)) ;% Power output per leg
   

    Qhn(j) = abs(snm(j)).*Tempr(j+C2).*I2 + cn(j).*(Tempr(j+C2)-Tempr(j+1+C2)) - (0.5).*(I2.^2).*rn1(j) - 0.5.*(thnm(j))*I ; % Heat input to hot side
    Qcn(j) = abs(snm(j)).*Tempr(j+1+C2).*I2 + cn(j).*(Tempr(j+C2)-Tempr(j+1+C2)) + 0.5.*(I2.^2).*rn1(j) + 0.5.*((thnm(j)))*I ;% Heat rejection from cold side
