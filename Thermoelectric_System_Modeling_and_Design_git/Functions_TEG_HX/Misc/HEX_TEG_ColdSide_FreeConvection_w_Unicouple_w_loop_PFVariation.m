...
tnv =  2.6*(10^-3): (0.05*(10^-3)): 4.0*(10^-3);
leng = length(tnv)
%%
hp = 2*(10^-3);
hn = hp;
...
hxl  = 0.04; %m
hxlC = hxl;
Cb = 10*10^-3; % m Cold Base Thickness
cvl = hxl; 
...
Tcg = 2; % [C] Guess for temperature of TEMC
Tcbg = 85; %  [C] Guess for temperature of Fin Base
ErrMult = 3.5;
...
save('PD_DOE_PlateFinHS_LEgPF_COnstH')
