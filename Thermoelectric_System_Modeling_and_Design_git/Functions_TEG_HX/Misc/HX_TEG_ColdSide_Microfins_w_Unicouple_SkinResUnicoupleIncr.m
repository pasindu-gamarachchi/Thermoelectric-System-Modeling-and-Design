...
hnv = (0.3*10^-3):(0.025*10^-3):0.6*10^-3
length(hnv)

%%
% Guesses and Error
Err = (10^-4);% 0.01; % Convergence Error
Tcg = 0.04; % [C] Guess for temperature of TEMC
Tcbg = 0.5; %  [C] Guess for temperature of Fin Base
...
F = 5; % Couples Per Control Volume
E = floor(hxwC./(tcw));
...
[Qc, hC, T, Gr] =HXMicrofins(Tb, Tamb);

LegPackFrac = (ln*tn*2*C)/(hxlC*hxwC)
% Rth = (Tb-Tamb)/Qc;
LegPackFracv(a) = LegPackFrac
%%
    ...
    while  abs(Qc -QTEC)> Err  ||  abs(QHand - QTEH) > Err
        ...
        [Qc, hC, TEMC, Gr] = HXMicrofins(Tb, Tamb);%mean([Tcf(j), Tcf(j+1)]) );
...
[Y I] = max(TotalPv)
Rat_MaxPower =Ratio(I)
HeatIn_MaxPower = QTEHv(I)
LegH_MaxPower = hnv(I)
Volt_MaxPower = Vv(I)
HotT_MaxPower = TEMHv(I)
ColdT_MaxPower = TEMCv(I)
