...
Tbg = 80;  % Base/TEM Hot Side Temperature
TempCh = Err*0.9;
ErrMult = 5;
DiffLim = 1;
...
%     err = Tog- To;
    ...
    while abs(Qf-QTET)>Err
        ...
        Tb = Tb+(Err);
        [h, v, Re, f_f, prD]= convcoeffv2(T(j),mf);
        M = (sqrt(h*Pf*kNim*Af))*((Ti- Tb));
        m = sqrt((h*Pf)/(kNim*Af));
        Qf = M*tanh(m*hxh/2)*((N-1)) + Un_ar*h*(Tb - Tfl) ;
        QTET = TEBlock(Tb,TempC);
...
name = ['tf_', num2str(tfmm), '_PF', num2str(PF), 'cv_ ', num2str(cv), '_ModelVerification_'];
