...
global hxl hxh hxw N Tin tf TEMW TEML tc sl cs cw E TempC cvl Thg NC tf_C mfC
% HX Dimensions & Input
hxl = 0.16; % [m]
hxh = 0.02; % [m]
hxw = 0.04; % [m]
...
Tbg = 53;  % Base/TEM Hot Side Temperature
Tcfg= 5;
...
%         TempC = TempC -0.1;
        ...
        [Qc ToutC hC] = HXColdWater(TempC, mean([Tcf(j), Tcf(j+1)]) );
       
        if Qc -QTE> Err %& j ==1   %Qc -QTE> Err & j ==1
          TempC = TempC -(Err*0.2);
        ...
        if (Qf-QTE)>Err  %& j ==1   % (Qf-QTE)>Err  & j ==1
          Tb = Tb+(Err*0.2);
        elseif  (Qf- QTE)< -Err 
            Tb = Tb -(Err*0.1);
        end
