...
global hxl hxh hxw N Tin tf TEMW TEML tc sl cs cw E TempC cvl Thg NC tf_C mfC Tcg Tcbg
% HX Dimensions & Input
hxl = 0.16; % [m]
hxh = 0.02; % [m]
hxw = 0.04; % [m]
...
Tbg = 61;  % Base/TEM Hot Side Temperature
Tcfg= 0.5;
Tcg = Tcfg;
Tcbg = TempC -TCoIn;
...
%     err = Tog- To;
    ...
    while ( abs(Qc -Qf)> Err || abs(Qf-QTE)> Err  ) % Or condition used , because both conditions have to be satisfied, if and is used when either is satisfied loop will stop
...
%         TempC = TempC -0.1;
        ...
        [Qc ToutC hC vC] = HXColdWater(TempC, Tcf(j));%mean([Tcf(j), Tcf(j+1)]) );
       
        if Qc -QTE> Err & Qc -QTE< (Err*10)   %Qc -QTE> Err & j ==1
          TempC = TempC -(Err*0.2);
        elseif Qc - QTE> (Err*10) 
            TempC = TempC -(Err*4.0);
        elseif (Qc -QTE) < (Err*-1)  %abs(Qc -QTE) > (Err*1) 
            TempC = TempC +(Err*0.2);
        
        end
        
        if (Qf-QTE)>Err & Qf -QTE< (Err*10)   %& j ==1   % (Qf-QTE)>Err  & j ==1
          Tb = Tb+(Err*0.2);
        elseif Qf - QTE> (Err*10) 
            Tb = Tb +(Err*4.0);
        elseif  (Qf- QTE)< -Err 
            Tb = Tb -(Err*0.1);
        end
        ...
        if ( abs(Qc -QTE) +abs(Qf-QTE) ) < Err*2.2
            break
        else
        end
        if Qc<0
