...
% Version 0 on 05/17, with iterative Ra


global hxl  TempC cvl Thg NC tf_C mfC Tcg Tcbg hxhC hxwC hxlC Cb hxw
...
Tcbg = 5; %  [C] Guess for temperature of Fin Base
ErrMult = 5.5;
...
Rth = (Tb-Tamb)/Qc
    ...
    while ( abs(Qc -QTE)> Err  )
         ...
         QTE = TEBlock(Th, TempC);
        [Qc hC TEMC Ra Tcbtr] = HXFreeConvV2(Tb, Tamb)%mean([Tcf(j), Tcf(j+1)]) );
