...
Tamb= 20;
hxhCV = linspace(0.01,0.15, 15);
NCV = 2:1:25;
tf_Cv = linspace(0.05*10^-3,1.2*10^-3,2);
%%

for i = 1: length(hxhCV)
    for j = 1:length(NCV)
%         for k = 1: length(tf_Cv)
...
G  = floor(hxl./(cvl)); % Number of unicouples along fin direction
...
% Guesses and Error
Err = 0.01; % Convergence Error

Tcg = 0.004; % [C] Guess for temperature of TEMC
Tcbg = 180; %  [C] Guess for temperature of Fin Base
ErrMult = 5.5;
...
% Rth = (Tb-Tamb)/Qc
    ...
    while ( abs(Qc -QTE)> Err  )
         ...
         Q = TEBlock(Th, TempC);
         QTE = Q;
        [Qc hC TEMC Ra Qr] = HXFreeConvRad(Tb, Tamb);%mean([Tcf(j), Tcf(j+1)]) );
...
progress = (i*j)/(length(tf_Cv)*length(NCV))
    end
