...
% Free Convection
% Iteration method applied to Ra # on 05/16


 function [Qc h TEMC Ra Tcb_c] = HXFreeConvV2(Tcb, Tamb)
...
h = (Nu*kam)/sC;
% h =40;
 M = (sqrt(h*Pf*k*Af))*(Tb -Tamb);
m = sqrt((h*Pf)/(k*Af));

A = NC*hxlC*hxhC *2 + ( sC*hxlC*(NC-1)) + (2*tf_C*hxlC);
Qc = M*tanh(m*hxhC)*(NC);

Tcb_c = Qc/(h*A) +Tamb;
%%
while abs(Tcb_c- Tb)> ConvErr
